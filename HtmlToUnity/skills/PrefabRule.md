...
```
XxxWindow            Canvas + CanvasScaler + GraphicRaycaster + XxxWindow.cs
...
**절대 y 를 박기보다 원본 flex 를 그대로 옮기는 게 낫다.** 블록 높이만 실측해 넣으면
접힘/펼침 같은 상태 변화가 자동으로 맞는다.

**패널이 형제 패널의 `RectTransform` 을 직접 고치지 않는다.** 자기 구획만 바꾸고
남는 높이는 부모 레이아웃이 나눠 준다 (원본이 flex 로 하는 일).
...
## 프리팹은 에디터에서 만든다 — 코드로 굽지 않는다

**이관 중 프리팹을 코드로 굽는 빌더를 두었다면, 이관이 끝나는 시점에 걷어낸다.**
되살리지 않는다.

이유는 하나다 — **아트팀이 같은 프리팹을 만진다.** 코드가 프리팹을 다시 구우면
그 작업이 통째로 사라진다. 재현성보다 협업이 이긴다.
