...
| 어디에 | 무엇 |
|---|---|
| **창 루트** | 배경 · 덮개. **화면을 꽉 채운다** (안전 영역을 적용하지 않는다) |
| **그 아래 안전 영역 자리** | 글자 · 버튼 · 판 등 **콘텐츠 전부** |

⚠ **배경에 안전 영역을 걸면 노치 기기에서 가장자리에 빈 띠가 생긴다.**
가려지면 안 되는 것과 꽉 차야 하는 것은 **다른 자리**다.

⚠ **에디터에서는 안전 영역이 화면 전체라 아무 일도 안 일어난다.**
있는지 없는지를 눈으로 못 보므로 **배선 검사가 「창마다 있다」를 기계로 확인**한다.
