...
**이관하는 게임이 로컬에 무엇이든 남기면(최고 기록 · 해금 · 설정 · 진행), «지우는 버튼»을 반드시 같이 만든다.**

⚠ **적용은 «앞으로»다 — 이미 끝난 게임을 소급해서 고치지 않는다.**
끝난 게임을 건드리려면 UI 프리팹을 다시 구워야 하고, 그건 **검증을 통과해 둔 결과물을 다시 흔드는 일**이다.
(**[예시]** 이 저장소 — Undead Slayer 부터 단다. 앞서 끝낸 게임은 그대로 둔다.)

**왜** — 저장이 쌓이면 **「처음 켠 사람」의 화면을 다시 볼 수 없다.**
첫 진입 연출 · 잠긴 화면 · 초기 문구는 **그 상태에서만** 원본과 대조할 수 있고,
사람이 확인하려 할 때도 되돌릴 길이 없으면 **엔진을 열어 지우라고 시키는 수밖에 없다**
(「사람에게 엔진 조작을 요구하지 않는다」를 어기게 된다).

| 무엇 | 어디 |
|---|---|
| 지우는 일을 모으는 자리 | **공용** `Assets/Scripts/Core/SaveWipe.cs` |
| 그 게임이 «무엇을 지우나» | **게임 쪽** — 키를 아는 곳이 지운다 |
| 버튼 | 그 게임의 **첫 화면 구석에 작게**. 원본 요소와 헷갈리지 않게 연출을 안 건다 |

⚠ **키 목록이 아니라 «지우는 일»을 등록한다** — `World_3` 처럼 **이름이 그때그때 만들어지는 키**가 있어서
바깥에서 키를 다 알 수 없다. 대신 **대표 키**를 같이 넘겨 **검사가 「정말 지워졌나」를 보게** 한다
(`PlayerPrefs` 는 키를 훑을 수 없다).

⚠ **이것은 «원본에 없는 것»이다** — 그 게임 확정표의 <b>의도된 차이</b>에 적는다.
문구도 **원본 로케일 표에 넣지 않는다** — 그 표는 «원본 전수»가 분모라 우리 것을 섞으면 이관율이 거짓이 된다.

⚠ **등록을 빠뜨리면 버튼이 있어도 아무것도 안 지운다 — 그런데 오류는 안 난다.**
⇒ 회귀 검사가 **「눌러서 키가 사라졌나」**까지 본다 (붙였다고 눌리는 게 아니다 · `재발방지 #159`).
...
### ★★ 저장소에 게임이 여럿이면 — **빌드 «직전»에 그 게임 것을 적용한다**

**프로젝트 설정은 하나뿐이다.** 확정표를 설정에 넣는 코드가 있어도
**빌드가 그것을 안 부르면 「마지막에 만진 게임」의 설정이 그대로 나간다** —
방향 · 패키지 이름 · 제품명 · **시작 씬**까지 전부.

⚠ **에디터에서는 잘 돈다.** 구워서 기기에 올리기 전까지 아무도 모른다.

| 겹 | 무엇 |
|---|---|
| **① 게임별 빌드 진입점** | 확정표 적용 → **씬을 그 게임 것만** → **되물어 확인** → 빌드 |
| **② 빌드 전처리** | 사람은 **Build 버튼을 누른다.** 그때 **시작 씬 경로로 게임을 알아내** 같은 것을 적용한다 |
