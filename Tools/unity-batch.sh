...
# ★★ 재생 검사도 «다리»로 보낸다 — 에디터를 껐다 켜지 않는다.
#
#   예전에는 「뒤에 있는 에디터의 재생은 틱이 느리다(실측 7분 무응답)」를 이유로 배치로 돌렸다.
#   ⚠ 그 느림의 원인은 다리가 아니라 **`Application.runInBackground` 가 꺼져 있던 것**이다 —
#     에디터는 포커스를 잃으면 재생을 throttle 하고, 셸이 앞에 있으면 «항상» 그렇다.
#     검사기가 시작할 때 그 값을 켜자 배치와 같은 속도로 돈다.
#
#   그래서 기본이 다리다. 배치가 필요하면 BRIDGE_PLAY=0 (또는 CLOSE_EDITOR=1) 으로 되돌린다.
if editor_running && [ "${BRIDGE:-1}" = "1" ] && [ "${CLOSE_EDITOR:-0}" != "1" ] && { [ "${PLAYMODE:-0}" != "1" ] || [ "${BRIDGE_PLAY:-1}" = "1" ]; }; then
