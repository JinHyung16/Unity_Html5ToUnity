...
| **재생(Play) 검사** | **된다** | `PLAYMODE=1 Tools/unity-batch.sh …` — **배치가 필수인 유일한 검사**다(뒤에 있는 에디터의 재생은 틱이 느려 검사가 멈춘다). 러너가 **저장 후 정상 종료 → 배치 → 복구 백업을 치우고 다시 연다**(사람이 누를 대화상자 없음). ⚠ 검사기는 `SessionState "JinHyung.EditorBridge.Driving"` 이 참이면 `Exit` 를 부르지 않는다(재발방지 #140). ⚠ **입력·레이캐스트는 여기서만 잰다** |
...
**러너 스위치** — `CLOSE_EDITOR=1` (닫고 시작 — **저장 후 정상 종료**를 먼저 시킨다 · `SHUTDOWN_TIMEOUT`) · `REOPEN=1` (끝나고 연다 — 복구 백업을 치워 「_recovery backup scene?」 을 «No» 로 처리) · `BRIDGE=0` · `PLAYMODE=1`.
